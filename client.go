...
type AgentConfig struct {
	// MaxIterations caps the number of model→tool round-trips.
	// 0 uses the default of 10.
	MaxIterations int
}

// RunAgent runs an agentic loop: call the model, execute any tool calls, feed
// results back, and repeat until the model produces a final text response or
// MaxIterations is reached. tools must contain the schemas the model may call;
// toolFn executes the actual tool logic.
//
// Returns the model's final text content and the full conversation history
// (including all tool call / tool result turns).
func (c *Client) RunAgent(
	...
	for i := 0; i < cfg.MaxIterations; i++ {
		...
		for _, tc := range resp.ToolCalls {
			result, err := toolFn(ctx, tc.Function.Name, tc.Function.Arguments)
			if err != nil {
				return "", msgs, fmt.Errorf("tool %q: %w", tc.Function.Name, err)
			}
			msgs = append(msgs, providers.ToolResultMessage(tc.ID, tc.Function.Name, result))
		}
	}

	return "", msgs, fmt.Errorf("agent: reached max iterations (%d)", cfg.MaxIterations)
}
...
// silently between turns. The channel is closed when the agent finishes or
// an error occurs (delivered as a chunk with Error set).
//
// The full conversation history is sent to histCh (buffered 1) when done;
...
func (c *Client) RunAgentStream(
	...
	go func() {
		...
		for i := 0; i < cfg.MaxIterations; i++ {
			...
			// Collect the streamed response, forwarding content chunks to caller.
			...
			for chunk := range ch {
				...
				if chunk.Done {
					...
					// Forward the Done chunk so callers know this turn ended.
					if !send(chunk) {
						return
					}
				}
			...
			for _, tc := range toolCalls {
				result, err := toolFn(ctx, tc.Function.Name, tc.Function.Arguments)
				if err != nil {
					send(&providers.StreamChunk{
						Error: fmt.Errorf("tool %q: %w", tc.Function.Name, err),
						Done:  true,
					})
					return
				}
		...
		send(&providers.StreamChunk{
			Error: fmt.Errorf("agent: reached max iterations (%d)", cfg.MaxIterations),
			Done:  true,
		})
	}()
...
func agentDefaults(opts []AgentConfig) AgentConfig {
	cfg := AgentConfig{MaxIterations: 10}
	if len(opts) > 0 && opts[0].MaxIterations > 0 {
		cfg.MaxIterations = opts[0].MaxIterations
	}
	return cfg
}
